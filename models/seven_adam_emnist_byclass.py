...
def load_data(mat_file_path, width=28, height=28, max_=None, verbose=True):
    ...
    pickle.dump(mapping, open('bin/sevenadam_mapping.p', 'wb' ))
...
def train(model, training_data, callback=True, batch_size=256, epochs=10):
    ...
    if callback == True:
        ...
        tbCallBack = keras.callbacks.TensorBoard(log_dir='./SevenAdamByClassGraph', histogram_freq=0, write_graph=True, write_images=True)
    ...
    with open("bin/sevenadam_model.yaml", "w") as yaml_file:
        yaml_file.write(model_yaml)
    save_model(model, 'bin/sevenadam_model.h5')
